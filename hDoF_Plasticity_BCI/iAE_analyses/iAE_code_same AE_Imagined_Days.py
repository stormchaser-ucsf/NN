# -*- coding: utf-8 -*-
"""
Created on Sat Nov 12 11:12:46 2022
...
for days in (np.arange(10)+1):
    ...
    for loop in np.arange(iterations):
        ...
        # train the model
        Ytest = np.zeros((2,2))
        #condn_data_online = np.concatenate((condn_data_online,condn_data_batch),axis=0)
        #Yonline = np.concatenate((Yonline,Ybatch),axis=0)
...
N=1
X=np.arange(10)+1
tmp1=var_overall_imagined_days[0]
...
plt.figure();
tmp = np.squeeze(mahab_distances_imagined_days)
tmp = np.mean(tmp,1)
plt.plot(tmp);
