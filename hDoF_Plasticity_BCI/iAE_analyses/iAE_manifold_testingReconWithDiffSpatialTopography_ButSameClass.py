# -*- coding: utf-8 -*-
"""
Created on Sun Jul 23 22:19:41 2023
...
for days in (np.arange(num_days)+1):
    ...
    # small changes to combine classes to check for reconstruction
    idx = np.where(Yimagined[:,6]==1)[0] #last 300
    Yimagined[idx,6]=0;
    Yimagined[idx,5]=1
    Yimagined = Yimagined[:,:6]
    
    idx = np.where(Yonline[:,6]==1)[0] #578 onwards
    Yonline[idx,6]=0;
    Yonline[idx,5]=1
    Yonline = Yonline[:,:6]
    
    idx = np.where(Ybatch[:,6]==1)[0] #292 onwards
    Ybatch[idx,6]=0;
    Ybatch[idx,5]=1
    Ybatch = Ybatch[:,:6]
