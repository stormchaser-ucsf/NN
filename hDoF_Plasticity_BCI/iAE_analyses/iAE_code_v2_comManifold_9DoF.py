# -*- coding: utf-8 -*-
"""
Created on Fri May 12 10:04:35 2023
...
# main loop , load the data for both days together 
condn_data_imagined_total=np.empty((0,96))
Yimagined_total = np.empty((0,9))
condn_data_online_total=np.empty((0,96))
Yonline_total = np.empty((0,9))
condn_data_batch_total=np.empty((0,96))
Ybatch_total = np.empty((0,9))
...
for days in (np.arange(num_days)+1):
    ...
    # append across days
    condn_data_imagined_total = np.concatenate((condn_data_imagined_total,condn_data_imagined),axis=0)
    condn_data_online_total = np.concatenate((condn_data_online_total,condn_data_online),axis=0)
    condn_data_batch_total = np.concatenate((condn_data_batch_total,condn_data_batch),axis=0)
    Yimagined_total = np.concatenate((Yimagined_total, Yimagined),axis=0)
    Yonline_total = np.concatenate((Yonline_total, Yonline),axis=0)
    Ybatch_total = np.concatenate((Ybatch_total, Ybatch),axis=0)
    
condn_data_imagined = condn_data_imagined_total
condn_data_online = condn_data_online_total
condn_data_batch = condn_data_batch_total
Yimagined = Yimagined_total
Yonline = Yonline_total
Ybatch = Ybatch_total
nn_filename = 'iAE_9dof' + '.pth'
...
tmp_batch=np.squeeze(np.mean(var_batch_days,axis=1))[:,0]
tmp_online=np.squeeze(np.mean(var_online_days,axis=1))[:,0]
tmp_imagined=np.squeeze(np.mean(var_imagined_days,axis=1))[:,0]
...
print(stats.ttest_rel(tmp_imagined,tmp_batch))
print(stats.ttest_rel(tmp_batch,tmp_online))

tmp_imagined = np.squeeze(np.mean(mahab_distances_imagined_days,axis=1))[:,0]
tmp_online = np.squeeze(np.mean(mahab_distances_online_days,axis=1))[:,0]
tmp_batch = np.squeeze(np.mean(mahab_distances_batch_days,axis=1))[:,0]
