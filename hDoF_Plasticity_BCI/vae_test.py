...
condn_data_online = np.concatenate((Xtrain,Xtest),axis=0)
Yonline = np.concatenate((Ytrain,Ytest),axis=0)
D = plot_latent(model_goat, condn_data_online,Yonline,int(1e4),3)

# load online data and project into it 
file_name = 'F:\DATA\ecog data\ECoG BCI\GangulyServer\Multistate clicker\condn_data_onlineDay4.mat'
data_dict = mat73.loadmat(file_name)
data_imagined = data_dict.get('condn_data')
#condn_data_imagined = np.array(condn_data_imagined)
...
for i in range(Y.shape[0]):
    ...
    Y_mult[i,tmp]=1
Y = Y_mult
D = plot_latent(model_goat, condn_data_online,Y,int(1e4),3)
